...
def test_payments_summary_no_service_fee_when_flag_off(monkeypatch) -> None:
...
def test_payments_summary_service_fee_when_flag_on(monkeypatch) -> None:
...
def test_payments_summary_no_refund_total_when_flag_off(monkeypatch) -> None:
    monkeypatch.setattr(flags, "enabled", lambda name: False)
    resp = client.get("/payments/summary")
    assert resp.status_code == 200
...
def test_payments_summary_refund_total_when_flag_on(monkeypatch) -> None:
